...
class Sidebar extends React.Component {
  ...
  render() {
    ...
    let allResults;
    let numPages =
      Math.floor((photos.length - 1) / 10) < 0
        ? 0
        : Math.floor((photos.length - 1) / 10);
    let pages = {};
    let sidebarFilter;
    let allContent;
    let favoriteContent;
      ...
      <div className="dropdown-filter">
        ...
        <div id="myDropdown" className="dropdown-content-filter">
          <button
            ...
            onClick={() => this.setState({ filter: "All Content" })}
          >
            All Content
          </button>
          <button
            className="filter-item"
            onClick={() => this.setState({ filter: "Favorites" })}
          >
            Favorites
          </button>
        </div>
    ...
    if (photos.length <= 10) {
      allResults = (
        <button disabled className="all-results">
          All Results
        </button>
      );
    }
